...
namespace TechnologicalUI
{
    public class Authorization
    {
        ...
        public void LogIn(string login, string password)
        {
            try
            {
                var _context = new DB.ApplicationContext(Connection.GetConnectionString(_config, 0, 0));
                BL.Facade  _facade = new BL.Facade(new RepositoriesFactory(_context));

                BL.User user = _facade.LogIn(login, password);
                OpenConnection(Connection.GetConnectionString(_config, 0, (int) user.UserType), _context, user);
            }
            catch (Exception exc)
            {
